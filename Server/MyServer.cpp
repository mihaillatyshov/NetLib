...
void MyServer::SendMyMessage(const std::string& MyMessage)
{
	std::shared_ptr<Packet> MyPacket = std::make_shared<Packet>(PacketType::PT_ChatMessage);
	*MyPacket << MyMessage;
	for (auto& connection : connections)
	{
		connection.pm_outgoing.Append(MyPacket);
	}
}
